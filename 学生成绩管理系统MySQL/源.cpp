...
start:
	...
	if (mysql_query(&sql.mysql, query.c_str()))
	{
		...
		color(12);
		gotoxy(10, 8);
		cerr << "录入失败:" << mysql_error(&sql.mysql) << endl;
		color(11);
		gotoxy(10, 10);
		system("pause");
		return;
		...
		else
		{
			...
			if (chose == 'Y' || chose == 'y')
			{
				if (mysql_query(&sql.mysql, query.c_str()))
				{
					...
					color(12);
					gotoxy(10, 7);
					cout << "删除失败:" << mysql_error(&sql.mysql) << endl;
					color(11);
					gotoxy(10, 9);
					system("pause");
					mysql_free_result(result);
	...
	start:
		...
		else
		{
			...
			if (mysql_query(&sql.mysql, query.c_str()))
			{
				...
				color(12);
				gotoxy(10, 8);
				cerr << "修改失败:" << mysql_error(&sql.mysql) << endl;
				color(11);
				gotoxy(10, 9);
				system("pause");
			}
...
void Print_allstu(bool mode)
{
	...
	{
		if (mode == true)
			mysql_query(&sql.mysql, "SELECT * FROM STU ORDER BY ID ASC;");	//以学号正序打印
		else
			mysql_query(&sql.mysql, "SELECT * FROM STU ORDER BY sum DESC;");//以成绩倒序打印
		MYSQL_RES*result = mysql_store_result(&sql.mysql);
		MYSQL_ROW row;
		GUI_2();
		color(14);
